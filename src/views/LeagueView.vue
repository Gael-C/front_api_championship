    ...
    <div class="md:flex items-start justify-center py-12 2xl:px-20 md:px-6 px-4" v-if="getLeague">
      ...
      <div class="xl:w-2/5 md:w-1/2 lg:ml-8 md:ml-6 md:mt-0 mt-6">
        <div class="border-b border-gray-200 pb-6">
          <h1 class="lg:text-2xl text-xl font-semibold lg:leading-6 leading-7 text-gray-800 dark:text-white mt-2">{{ getLeague.name }}</h1>
        </div>
        <detail :cle="'Création'" :valeur="getLeague.creation"/>
        <detail :cle="'Champion en titre'" :valeur="getLeague.last_champion"/>
        <detail :cle="'Club le plus titré'" :valeur="getLeague.most_successfull"/>
        <div v-for="teams in getLeague.teams">
